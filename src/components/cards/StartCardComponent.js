...
import BackButton from './BackButtonComponent.js';

require('styles/cards/StartCard.styl');

class StartCardComponent extends React.Component {
  render() {
    const {cards, nextCardLayer, prevCardLayer} = this.props;

    let basicAttribute = cards.layer > 0 ? (
      <BackButton name={'Back'} clickEvent={prevCardLayer}/>
    ) : (
    ...
    return (
      <div className="startcard-component">
        {basicAttribute}
        <AttributeList prevCardLayer={prevCardLayer} nextCardLayer={nextCardLayer} cards={cards}/>
